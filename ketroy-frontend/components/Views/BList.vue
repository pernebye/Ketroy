        ...
        <template v-for="(item, index) of list.filter((_, index) => index >= (page - 1) * perPage && index < page * perPage)" :key="index">
          <div class="admin-list-item">
            <div class="admin-list-item__image">
              <fade>
                <img
                  v-if="item && item.image && typeof item.image === 'string' && item.image.includes('/images')"
                  ...
                  draggable="false"
                />
                <video v-else-if="item.image && item.image.path && item.image.path.includes('/videos')" class="admin-list-item__video" :src="item.image.path"></video>
            ...
            <div class="admin-list-item__content">
              <div class="admin-list-item__info">
                <span class="admin-list-item__title">{{ String(item.title).slice(0, 60) }}</span>
                <expand>
              ...
              <div class="admin-list-item__actions">
                <fade>
                  <NuxtLink v-if="item.title" class="admin-list-item__action" :to="{ name: getDynamicRouteName(), query: { title: item.title } }">
                    <button class="admin-list-item__btn">
                      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path
                ...
                <fade>
                  <button v-if="showDeleteBtn" class="admin-list-item__btn admin-list-item__btn--delete">
...
const getDynamicRouteName = () => {
  if (route.name.includes('categories')) {
    return 'categories-create';
  } else if (route.name.includes('actuals')) {
...
.admin-list-item__btn svg {
  width: 100%;
  height: 100%;
}

.admin-list-item__btn:hover {
  transform: scale(1.1);
}

.admin-list-item__btn--delete {
  color: #ef4444;
}
